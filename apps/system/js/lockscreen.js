...
(function(exports) {
  ...
  function ls_init() {
    this.ready = true;
    this._unlocker = new self.LockScreenSlide();
    this.getAllElements();
    ...
    if (window.navigator.mozMobileConnections) {
      this._lockscreenConnInfoManager =
        new self.LockScreenConnInfoManager(this.connStates);
    }

    /* media playback widget */
    this.mediaPlaybackWidget =
      new self.MediaPlaybackWidget(this.mediaContainer);

    self.SettingsListener.observe('lockscreen.enabled', true,
      (function(value) {
        this.setEnabled(value);
    }).bind(this));

    var wallpaperURL = new self.SettingsURL();

    self.SettingsListener.observe('wallpaper.image',
    ...
    self.SettingsListener.observe('lockscreen.unlock-sound.enabled',
      true, (function(value) {
      this.setUnlockSoundEnabled(value);
    }).bind(this));

    self.SettingsListener.observe('lockscreen.passcode-lock.timeout',
      0, (function(value) {
  ...
  */
  ...
  function ls_lockIfEnabled(instant) {
    if (self.FtuLauncher && self.FtuLauncher.isFtuRunning()) {
      this.unlock(instant);
      return;
    }
  ...
  function ls_unlock(instant, detail) {
    ...
    var app = self.AppWindowManager ?
      self.AppWindowManager.getActiveApp() : null;
  ...
  function ls_lock(instant) {
    ...
    screen.mozLockOrientation(self.OrientationManager.defaultOrientation);
  ...
  function ls_writeSetting(value) {
    ...
    self.SettingsListener.getSettingsLock().set({
